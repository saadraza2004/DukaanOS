...
public class ProductService : IProductService
{
    ...
    public async Task<ProductDto> CreateProductAsync(CreateProductRequest request, int userId, int storeId = 1)
    {
        ...
        var product = new Product
        {
            ...
            Name = string.IsNullOrWhiteSpace(request.Name) ? (request.RomanUrduName?.Trim() ?? "Product") : request.Name.Trim(),
            UrduName = string.IsNullOrWhiteSpace(request.UrduName) ? (request.RomanUrduName?.Trim() ?? request.Name.Trim()) : request.UrduName.Trim(),
            RomanUrduName = string.IsNullOrWhiteSpace(request.RomanUrduName) ? request.Name.Trim() : request.RomanUrduName.Trim(),
